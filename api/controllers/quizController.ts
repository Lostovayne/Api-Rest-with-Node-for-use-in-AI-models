...
export const generateQuizForModuleController = async (
  req: Request,
  res: Response,
) => {
  ...
  try {
    ...
    await queueService.sendToQueue(
      rabbitmqConfig.queues.taskQueue,
      JSON.stringify(task),
    );
    ...
    console.error(
      `Error publishing quiz generation task for module ${moduleId}:`,
      error,
    );
    res
      .status(500)
      .json({ error: "Internal server error while starting quiz generation" });
  }
};

export const submitQuiz = async (req: Request, res: Response) => {
  const { quizId } = req.params;
  const { userId, answers }: { userId: number; answers: UserAnswer[] } =
    req.body;

  if (!userId || !answers || !Array.isArray(answers) || answers.length === 0) {
    return res
      .status(400)
      .json({ error: "userId and a non-empty answers array are required" });
  }
  ...
  try {
    ...
    const results = answers.map((answer) => {
      const isCorrect =
        correctAnswers.get(answer.questionId) === answer.selectedOptionIndex;
      if (isCorrect) {
        correctCount++;
      }
      return {
    ...
    res
      .status(500)
      .json({ error: "Internal server error while submitting quiz" });
  } finally {
    client.release();
  }
};
...
export const getUserPerformance = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error while getting user performance" });
  }
};
