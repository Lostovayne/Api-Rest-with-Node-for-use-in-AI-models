...
export const handleGenerateTts = async (payload: TaskPayload) => {
  const { text, jobId, userId, moduleId } = payload;
  taskLogger.info({ jobId, userId, moduleId }, `Processing task: Generate TTS`);
  ...
  try {
    ...
    taskLogger.info(
      { jobId, audioUrl, moduleId },
      `Successfully generated and saved TTS audio to Vercel Blob.`
    );
  } catch (error) {
